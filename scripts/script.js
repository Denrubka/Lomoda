...
try{
    ...
    const createCard = ({id, brand, name, preview, cost, sizes}) => {
...
cartOverlay.addEventListener('click', event => {
    ...
    if(target.matches('.cart__btn-close') || target == cartOverlay) {
        modalClose();
    }
})

document.addEventListener('keyup', event => {
    if(cartOverlay.classList.contains('cart-overlay-open') && event.key === 'Escape') {
        modalClose();
    }
})
